    ...
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title">
                Updates For <?= $model->title ?> - Add a New Update&nbsp;
                <span class="header1">
                    <button id="icon1" class="btn btn-primary accordion-button-right">
                        <i class="fa fa-minus"></i>
                    </button>
                </span>
                <a href="<?= Url::to(['category/clean-view', 'id' => base64_encode($model->category_id)]) ?>"
                   title="Go Back" style="float: right; margin-right: 5px"
            ...
            <div class="row">
                <div class="content1">
                    <form id="update-form<?= $model->id ?>"
                          action="<?= Yii::$app->urlManager->createUrl(['update/project-new']) ?>" method="post">
...
<script>
    ...
    $('#update-form<?=$model->id?>').submit(function () {
        ...
        $.ajax({
            ...
            data: form.serialize(),
            success: function (data) {
                $('#update-fields<?=$model->id?>').load("../category/aucontent?id=<?=$model->id?>");
